...
import { FileText, Layout, BookOpen, Settings, Eye, PlusSquare } from 'lucide-react';
...
export default function EditorPage() {
  ...
  const { activePanel, setActivePanel, plantillaId, volverInicio, resetDocumento } = useDocumentStore();

  const plantilla = PLANTILLAS.find((p) => p.id === plantillaId);

  const handleNuevoDocumento = () => {
    if (window.confirm('¿Crear un nuevo documento? Se perderán los cambios no guardados.')) {
      resetDocumento();
      volverInicio();
      navigate('/');
    }
        ...
        <div className="flex items-center gap-2 px-4 py-3 bg-blue-700 text-white flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-sm font-bold leading-tight truncate">
              {plantilla?.nombre ?? 'Generador de Informes'}
            </h1>
            <p className="text-xs text-blue-200 truncate">
              {plantilla?.nombreCompleto ?? ''}
            </p>
          </div>
          <button
